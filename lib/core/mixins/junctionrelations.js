...
exports.RelationsByJunctionTable = {
  ...
  _sharedReducer: function(data,relation,storeRequest){
    var relationFieldName = this.junctionKeyName(storeRequest.bucket, storeRequest.primaryKey, relation.get('isToMany'));
    var oppositeFieldName = this.junctionKeyName(relation.bucket, relation.primaryKey, relation.get('isFromMany'));
  ...
  fetchRelation: function(storeRequest,records,relation,callback){
    ...
    var relationTable = relation.get('isDirectRelation')? this.junctionTableName(storeRequest.bucket,relation.bucket): relation.bucket;
      ...
      if(relation.isDirectRelation){
        ...
        if(relation.isChildRecord){      // go and get the data
          ...
          me.fetchDBRecords(childReq,C.SOURCE_THOTH,function(err,childRecordData){
            var newRelSet = { bucket: storeRequest.bucket, keys: [], propertyName: relation.propertyName, propertyKey: relation.propertyKey, data: {} };
    ...
    if(relation.keys && (relation.keys.get('length') > 0) ){
      primaryKeys = relation.keys;
    }
    else primaryKeys = records.getEach(storeRequest.primaryKey);
    
    //StoreRequest.fromRelation = function(relation,requestType,opts){
    if(relation.get('isToOne') && !(relation.keys instanceof Array) && relation.isChildRecord){
      ...
      relStoreReq = API.StoreRequest.create({
        ...
        params : { keys: primaryKeys }   // keys are the primaryKey values of records
      });
