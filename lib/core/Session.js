...
exports.Session = SC.Object.extend({
  ...
  // in: storeRequest
  // out: {user: curUser, sessionKey: curUserInfo.sessionKeys[j], matchType: isMatch}
  ...
  findEligableUserSessions: function(storeRequest,callback){
    ...
    var queryMatcher = function(sesq){
      if(!me._queryCache[sesq.conditions]) return false;
      if(!me._queryCache[sesq.conditions][sesq.parameters]) return false;
      return me._queryCache[sesq.conditions][sesq.parameters].query.contains(record);
    };
    ...
    var processSession = function(sesrec){
      if(sesrec.bucketKeys[bucket].contains(key)){
