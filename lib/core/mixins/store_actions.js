...
var StoreActionCreators = {
	...
	createStoreAction: function(APIRequest,userData,callback){
		...
		switch(AR.get('requestType')){ // was SR.action
			case C.ACTION_FETCH: ret = this._wrapFetch(AR,UD,CB);  break;
			case C.ACTION_REFRESH: ret = this._wrapRefresh(AR,UD,CB); break;
			case C.ACTION_CREATE: ret = this._wrapCreate(AR,UD,CB); break;
			case C.ACTION_UPDATE: ret = this._wrapUpdate(AR,UD,CB); break;
			case C.ACTION_DELETE: ret = this._wrapDelete(AR,UD,CB);break;
			default: break;
		}
		return ret;
	},
	...
	_wrapFetch: function(APIRequest,userData,callback){ //,requestType){
    ...
    var send = function(event,data){
      ...
      var ret = API.FetchResult.create({ 
        ...
        returnData: returnData
			...
			if(source === C.SOURCE_SOCKETIO){ 
			  storeRequest.records = records;
			  me.sessionModule.storeStoreRequest(storeRequest.userData.user, storeRequest.userData.sessionKey, storeRequest);
			}
			// send off the data
  ...
  _wrapRefresh: function(APIRequest,userData,callback){
  ...
  _wrapCreate: function(APIRequest,userData,callback){
		var me = this;
		//var userData = storeRequest.userData;
	...
	_wrapUpdate: function(APIRequest,userData,callback){  
  ...
  _wrapDelete: function(APIRequest,userData,callback){	
		var me = this, clientId, 
