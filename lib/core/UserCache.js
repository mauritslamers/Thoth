   ...
   */
   ...
   storeRecords: function(bucket,primaryKey,records){
      ...
      records.forEach(function(record){
         ...
         if(curkey) me.storeBucketKey(bucket,curkey, new Date().getTime()); 
      });
   ...
   deleteBucketKey: function(bucket,key){
      ...
      if(storedBucket){
         keys.forEach(function(k){
           if(storedBucket[k]) delete storedBucket[k];
         });
      } 
   },
   ...
   shouldReceive: function(storeRequest){
      ...
      var relationMatcher = function(r){
        ...
        return keys.some(function(k){
          if(bucketkeystore[r.bucket] && bucketkeystore[r.bucket][k]) return true;
          else return false;
        });
      };
