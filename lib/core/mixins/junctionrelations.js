...
// 
	_fetchJunctionRecords: function(mainPks,relationPks,junctionProps,callback){
		var me = this;
		var mainKeys = (SC.typeOf(mainPks) === 'array')? mainPks: [mainPks];
		var relationKeys = (SC.typeOf(relationPks) === 'array')? relationPks: [relationPks];

		var req = API.StoreRequest.create({
			requestType: C.ACTION_FETCH,
			bucket: junctionProps.junctionBucket,
			conditions: junctionProps.modelRelationKey + " ANY {keys} AND " + junctionProps.relationRelationKey + " ANY {relkeys}",
			parameters: { keys: mainKeys, relkeys: relationKeys }
		});
