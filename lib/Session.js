...
global.ThothSession = SC.Object.extend({
   
   sessionName: 'ThothNodeRiak', // lets choose some strange default
   ...
   createSession: function(userData,sessionKeyOnly){
      ...
      if(!this._loggedInUsers[user]){ // no existing info, create 
         ...
         if(this._loggedInUsers[user].sessionData instanceof Array){
            this._loggedInUsers[user].sessionData.push(ThothUserCache.create());            
         }
         else {
            this._loggedInUsers[user].sessionData = [ThothUserCache.create()];
         }
      }
      var sessionName = this.sessionName;
