...
exports.WrapperStore = SC.Object.extend({
  ...
  fetch: function(storeRequest,clientId,callback){
    if(this.stores && this.resources){
      var stores = this.storeForResource(storeRequest.bucket);
      if(stores.length === 1) stores[1].fetch(storeRequest,clientId,callback);
  ...
  refreshRecord: function(storeRequest,clientId,callback){
    if(this.stores && this.resources){
      var stores = this.storeForResource(storeRequest.bucket);
      if(stores.length === 1) stores[1].refreshRecord(storeRequest,clientId,callback);
  ...
  createRecord: function(storeRequest,clientId,callback){
    if(this.stores && this.resources){
      ...
      for(i=1,len=stores.length;i<len;i++){
        store[i].createRecord(storeRequest,clientId,function(){ return; }); //use an anonymous callback
      }
    }    
  },
  ...
  updateRecord: function(storeRequest,clientId,callback){
    if(this.stores && this.resources){
      ...
      for(i=1,len=stores.length;i<len;i++){
        store[i].updateRecord(storeRequest,clientId,function(){ return; });
      }      
    }
  },
  ...
  deleteRecord: function(storeRequest,clientId,callback){
    if(this.stores && this.resources){
      ...
      for(i=1,len=stores.length;i<len;i++){
        store[i].deleteRecord(storeRequest,clientId,function(){ return; });
      }      
    }    
  }
