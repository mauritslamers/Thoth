...
exports.RESTStore = Store.extend({

  automaticRelations: false,
  RESTHost: 'localhost',
  RESTPort: 80,
  RESTURLPrefix: '',
  useSSL: NO,
  ...
  protocol: function(){ 
    if(this.useSSL) return 'https://'; 
    else return "http://"; 
  }.property().cacheable(),

  urlHostPortAndPrefix: function(){ 
    if(this.RESTURLPrefix) return [this.RESTHost,":",this.RESTPort,"/",this.RESTURLPrefix,"/"].join("");
    else return [this.RESTHost,":",this.RESTPort,"/"].join("");
  }.property().cacheable(),

  fetchDBRecords: function(storeRequest,clientId,callback){
    //sys.log('fetching records ' + storeRequest.bucket.pluralize());
    var url = this.protocol() + this.urlHostPortAndPrefix() + storeRequest.bucket.pluralize();
    //sys.log('fetching records ' + url);
    var dataStore = ""; // temporary var to gather all data
    var httpClient = http.createClient(this.RESTPort,this.RESTHost); 
    var request = httpClient.request('GET',url,{ 'host': this.RESTHost});               
    ...
    request.addListener('response', function(response){
      ...
      response.addListener('end',function(){
        try {
          callback(null, JSON.parse(dataStore));
        } catch(e){
          sys.log('Bad data received from REST source: ' + JSON.stringify(dataStore));
        }
      });
    });            
  },
  
  createRESTRequest: function(method,url,storeRequest,clientId,callback){
    var dataStore = ""; // temporary var to gather all data
    var httpClient = http.createClient(this.RESTPort,this.RESTHost); 
    var request;

    // this is where the relations merging can be done, for backends that need them...
    //if (storeRequest.relations) {
      //storeRequest.relations.forEach(function(relation) {
      // ... 
      // ... use propertyName as keys
      // ...
      // ...
      //});
    //}
    ...
    if(data){
      ...
      sys.log('RESTStore: sending data to back end, method = ' + method + ': ' + dataToSend);
      ...
      sys.log('no data for ' + method);
    }
  },
  
  createDBRecord: function(storeRequest,clientId,callback){
    sys.log('createDBRecord, with storeRequest ' + sys.log(storeRequest));
    //sys.log('fetching records ' + storeRequest.bucket.pluralize());
    var url = this.protocol() + this.urlHostPortAndPrefix() + storeRequest.bucket;
    //sys.log('fetching records ' + url);
    this.createRESTRequest('POST',url,storeRequest,clientId,callback);
  },
  
  updateDBRecord: function(storeRequest,clientId,callback){
    sys.log('updateDBRecord, with storeRequest ' + sys.log(storeRequest));
    var url = this.protocol() + this.urlHostPortAndPrefix() + storeRequest.bucket + '/' + storeRequest.key;

    this.createRESTRequest('PUT',url,storeRequest,clientId,callback);
  },

  refreshDBRecord: function(storeRequest,clientId,callback){
    var url = this.protocol() + this.urlHostPortAndPrefix() + storeRequest.bucket + '/' + storeRequest.key;

    this.createRESTRequest('GET',url,storeRequest,clientId,callback);
  },
  
  deleteDBRecord: function(storeRequest,clientId,callback){
    var url = this.protocol() + this.urlHostPortAndPrefix() + storeRequest.bucket + '/' + storeRequest.key;

    this.createRESTRequest('DELETE',url,storeRequest,clientId,callback);
  },
...
exports.OrionDBStore = Store.extend(JunctionMixin,{
      // user functions
      ...
      updateDBRecord: function(storeRequest,clientId,callback){
         ...
         var request = this.createOrionDBHTTPRequest('PUT',storeRequest.bucket,storeRequest.key,storeRequest.recordData,function(data){
            ...
            if(parsedData) callback(null,parsedData[0]);
