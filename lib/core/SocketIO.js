...
exports.SocketIO = SC.Object.extend({
  ...
  _attachSessionCheckTimer: true, // to be able to turn off attaching a session check timer 
  ...
  _authWrapper: function(client){
    ...
    var createSesCB = function(err,sesrec){
      ...
      client.user = sesrec.username || sesrec.user; // sesrec is either sessionrecord (createSession) or userdata (checkSession)
      client.userData = sesrec.userData || sesrec;
      if(!client.userData.sessionKey) client.userData.sessionKey = sesrec.sessionKey;
      ...
      if(!me._attachSessionCheckTimer) return;
    ...
    var authSuccess = function(msg){
      ...
      if(sessionKey){ // try resume
        me.sessionModule.checkSession(msg,function(err,userdata){
          if(!userdata){
            me.sessionModule.createSession(msg,createSesCB);
          }
          else {
            createSesCB(null,userdata);
            me._sendRequestQueue(userdata.user,userdata.sessionKey);
          }
        });
      }
  ...
  _reauthWrapper: function(client){
    ...
    var reauthSuccess = function(userdata){
      client.userData = userdata;
      client.user = userdata.user;
      client.handshake.THOTH_sessionKey = userdata.sessionKey;
      client.handshake.THOTH_isAuthenticated = true;
      client.emit('authSuccess', {user: client.user, sessionKey: userdata.sessionKey });
      me._sendRequestQueue(userdata.user,userdata.sessionKey); // send request queue
      ...
      if(!me._attachSessionCheckTimer) return;
      client.sessionChecker = function(){
        me.sessionModule.checkSession({ user: userdata.user, sessionKey: userdata.sessionKey},function(hasSession){ return; });      
      };
      client.sessionCheckTimer = SC.Timer.schedule({
        target: client,
        interval: 15000,
    ...
    return function(data){  // re-authenticate... 
      if(data.user && data.sessionKey && me.sessionModule) { // perhaps also check password?
        ...
        me.sessionModule.checkSession(data,function(err,userdata){
          if(userdata) reauthSuccess(userdata);
  ...
  on: function(event,handler){
    ...
    // prevent anything else than a function being registered
    if(SC.typeOf(handler) !== 'function') return false; 
      ...
      return function(data){
        ...
        if(forceAuthentication){
          if(!client.userData){ // buffer 
            ...
            me.sessionModule.checkSession(client.userData,function(err,userdata){
              if(userdata) handler.call(context,apiRequest,userdata);
            });
          }
        }
        // at the moment you have to use forceAuthentication
        // the session is a very important part in the way Thoth works
        // distribution is not possible without it, so it is better to use some kind of
        // auth instead for the moment...
  ...
  clientFor: function(user,sessionKey){
    ...
    var s,c;
    if(!user || !sessionKey){
      sys.log('SocketIO.js; clientFor(): Warning: trying to look up a client (for distribution?) without a user or sessionKey?');
      sys.log('user: ' + user + ', sessionKey: ' + sessionKey);
      throw new Error('Trying to look up a client without a user or sessionKey');
    } 
