...
exports.Server = SC.Object.extend(httpResponses,StoreActionCreators,{
  ...
  // ============================
	...
	_checkDependencies: function(){
	  ...
	  try { var JSV = require('JSV'); }
	  catch(e){ 
	    throw new Error("Thoth needs JSV to function properly. Please install JSV using npm."); 
	  }
	},
...
/*			for(i=0,len=handlerFunctions.length;i<len;i+=1){
  ...
  destroyTemporaryURL: function(bucket,key){
	...
	onFetch: function(fetchReq,userData,callback){
		...
		if(this.checkJSONSchema){
		  sys.log("JSV validation: " + sys.inspect(this._jsvEnv.validate(fetchReq,API.APISCHEMAS[Constants.ACTION_FETCH]).errors));
		}
		
		storeRequest = API.StoreRequest.from(fetchReq,userData,callback);
		if(!storeRequest.get('isConsistent')){			
			callback(API.createErrorReply(Constants.ACTION_FETCH,Constants.ERROR_DATAINCONSISTENCY,returnData));
			return; 
		}

		fetchAction = this.createStoreAction(fetchReq,returnData,callback);

		if(this.policyModule) this.policyModule.checkPolicy(storeRequest,null,fetchAction);
		else fetchAction(YES);
	},
  
	onRefresh: function(refreshReq,userData,callback){
	  
		var returnData = refreshReq.returnData,
			storeRequest,	refreshAction,
			me = this;
		
		if(this.checkJSONSchema){
		  sys.log("JSV validation: " + sys.inspect(this._jsvEnv.validate(refreshReq,API.APISCHEMAS.retrieveRecord).errors));
		}				
		storeRequest = API.createStoreRequest(refreshReq,userData,Constants.ACTION_REFRESH);
  ...
  distributeChanges: function(storeRequest, originalUserData){
	...
	onCreate: function(createReq,userData,callback){
		...
		if(this.checkJSONSchema){
		  sys.log("JSV validation: " + sys.inspect(this._jsvEnv.validate(createReq,API.APISCHEMAS.createRecord).errors));
		}
   ...
   */
	...
	onUpdate: function(updateReq,userData,callback){
		...
		if(this.checkJSONSchema){
		  sys.log("JSV validation: " + sys.inspect(this._jsvEnv.validate(updateReq,API.APISCHEMAS.updateRecord).errors));
		}		
