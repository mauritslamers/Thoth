  ...
  getClient: function(){ // distribution of queues on clients...
    ...
    //this.log('client is: ' + sys.inspect(client));
		if(query){ 
		  ...
		  var cb = (!callback && params)? createCb(params): createCb(callback);
	...
	createClient: function(){
	  ...
	  if(this.user && (this.password !== null) && this.hostname && this.database){
	    ...
	    var child = cp.fork(Tools.libPath + '/node-mysql-client.js');
	    child.send({ 
	      isConnect: true,
  		  user      : this.user,
  		...
  		child.on('error',this.mySQLError);
