...
exports.Server = SC.Object.extend(httpResponses,StoreActionCreators,{
    ...
    // the order of the function names is the order of execution
		var handlerFunctions = [ 
		  ...
		  'handleStatusGET',
		  'handleAUTHRequest',
          'handleImagesUploadPOST',
          'handleImagesUploadProgressGET',
		  'handleSession'];
		...
		var handlers = {
            ...
            handleImagesUploadPOST: function(path,method,request,response,callback){
              var resource;
              if(method === 'POST'){
                resource = path.slice(1);
                if(resource && (resource.indexOf('images') === 0)){
                  serverObj.IMAGE_UPLOAD(request,resource,response);
			      callback('handleImagesUploadPOST', true);
                }
                else callback('handleImagesUploadPOST', false);
              }
			  else callback('handleImagesUploadPOST', false);
            },

			handleImagesUploadProgressGET: function(path,method,request,response,callback){
			  var resource;
			  if(method === 'GET'){
			    resource = path.slice(1);
			    if(resource && (resource.indexOf('progress') === 0)){
			      serverObj.IMAGE_UPLOAD_PROGRESS(request,response);
			      callback('handleImagesUploadProgressGET',true);
			    }
  			  else callback('handleImagesUploadProgressGET',false);
			  }
			  else callback('handleImagesUploadProgressGET',false);
			}
		};
...
/*			for(i=0,len=handlerFunctions.length;i<len;i+=1){
   ...
   IMAGE_UPLOAD: function(request,resource,response){
      // IMAGE_UPLOAD accepts a multipart request
      // call the IMAGE_UPLOAD
      // split the cacheKey from the resource
      sys.log("ThothServer: IMAGE_UPLOAD called");
      var resourceInfo = resource.split('/');
            ...
            this.tempStore.refreshDBRecord(this._createTempURLStoreRequest(bucket,key),null,function(what, rec){
              if (rec) { // something found, so parse it
                ...
                if (me._checkTempURLValidity.call(me,bucket,key,rec)) { // function auto-updates tempStore
                  ...
                  me.uploadModule.performImageUpload(request, function(data){
                    if (data) {
//
...
//
  ...
  IMAGE_UPLOAD_PROGRESS: function(request,response){
