  ...
  // ============================
	...
	_startServer: function(){
	  ...
	  if(this.useSSL){
	    ...
	    var cacert = this.SSLCACertFile? [fs.readFileSync(this.SSLCACertFile).toString()]: undefined;
	    this.server = https.createServer({
        key: fs.readFileSync(this.SSLKeyFile).toString(),
        cert: fs.readFileSync(this.SSLCertFile).toString(),
        ca: cacert
      }, this.createHTTPHandler(this));
	  } 
	  else this.server = http.createServer(this.createHTTPHandler(this));
		this.server.listen(this.port, function(){
		  sys.puts("Listening on port " + me.port);
		});
	...
	start: function(){
	  ...
	  process.title = (this.sessionModule && this.sessionModule.sessionName)? "Thoth_" + this.sessionModule.sessionName: "Thoth";
		...
		if(this.remoteREPL){
      this._remoteREPL = net.createServer(function(socket){
        require('repl').start("Thoth>",socket).context.myServer = me;
      }).listen(this.remoteREPLPort,'localhost');
		}
		if(!this.debug){
  ...
  onExit: function(){
	...
	_attachSocketListener: function(){
		...
		this.socketIO  = SocketIO.create({
		  ThothServer: this,
		  sessionModule: this.sessionModule,
...
/*			for(i=0,len=handlerFunctions.length;i<len;i+=1){
  ...
  STATUS: function(request,response){
  	...
  	var j,numnodes,node,k,retainer,size;
  	var statusText = "";
    ...
    if(this.isProfiling){
      var numlayers = 2;
      var childProcessor = function(el,layer){
        var i,child,k,retainer;
        if(el.retainersCount > 0){
          for(k=0;k<el.retainersCount;k+=1){
            retainer = el.getRetainer(k);
            size = retainer.to.retainedSize();
            if(size>50 && retainer.type !== 'Hidden'){
              sys.log('processing layer ' + layer);
                 statusText += "<tr>";
                 statusText += "<td>Layer " + layer + "</td>";
                 statusText += "<td>from " + retainer.from.name + " to " + retainer.to.name + "</td>";
                 statusText += "<td>type is: " + retainer.type + "</td>";
                 statusText += "<td>retainedSize: " + size + "</td>";
                 statusText += "</tr>";
            }
          }          
        }
        if(layer<numlayers){
             if(el.childrenCount > 0){
               for(i=0;i<el.childrenCount;i+=1){
                 childProcessor(el.getChild(i).to,layer+1);
               }
             }  
        }
      };
      // take snapshot, and then process it 
      //var snapshot = process.profiler.takeSnapshot();
      //var root = snapshot.root;
      //statusText += "<table border=1>";
      //childProcessor(root,0);
      //statusText += "</table>";
    }
  ...
  // callback is only here for the REST interface, when SOCKET.IO is detected, client.emit is being used
	onFetch: function(fetchReq,userData,callback){
		...
		if(!fetchReq.get('isConsistent')){			
			...
			callback(Constants.ACTION_FETCH, API.ErrorReply.from(Constants.ERROR_DATAINCONSISTENCY,fetchReq.get('returnData')));
			return; 
		}
  ...
  distributeChanges: function(storeRequest, originalUserData, rec){
    ...
    if(this.isBenchMarking) SC.Benchmark.start('distribute_process');
  	...
  	this.sessionModule.findEligableUserSessions(storeRequest, function(err,result){
  	  if(this.isBenchMarking) SC.Benchmark.end('distribute_eligability');
  	  if(err) return;
  	  result.forEach(determineActionType);
		...
		if(this.rpcModule){
			if(rpcRequest){
				...
				this.rpcModule.callRPCFunction(fn,params,function(data){
					...
					me.socketIO.emitTo(userdata,'rpcResult',data.rpcResult);
				});
			}
		}
		else {
		  ...
		  //callback(err);
		  me.socketIO.emitTo(userdata,'rpcError',err);
