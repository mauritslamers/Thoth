...
// ==========================================
  ...
  registerTemporaryURL: function(bucket,key,data,mimeType,limit,limitType){
    if(!this.tempStore){
    ...
    if(record){ // just in case
      switch(record.mimeType){
        case 'application/json': 
          ...
          break;
        default: 
          // assume a digital file
          response.writeHead(200, {'Content-Type': record.mimeType });
          response.write(record.data,'binary');
          response.end();
      }
    }
  },
   ...
   */
   ...
   onFetch: function(message,userData,callback){
      ...
      var storeRequest = { 
         ...
         action: 'refresh',
         primaryKey: fetchinfo.primaryKey,
         userData: userData,
         conditions: fetchinfo.conditions, 
         parameters: fetchinfo.parameters,
         properties: fetchinfo.properties,
   ...
   onRefresh: function(message,userData,callback){
      ...
      var storeRequest = { 
         ...
         primaryKey: refreshRec.primaryKey,
         action: 'refresh',
         userData: userData,
         key: refreshRec.key,
         properties: refreshRec.properties,
   ...
   onCreate: function(message,userData,callback){
      ...
      var storeRequest = { 
         ...
         key: createRec.key,
         primaryKey: createRec.primaryKey,
         action: 'create',
         userData: userData,
   ...
   */
   ...
   onUpdate: function(message,userData,callback){
     ...
     var storeRequest = { 
        ...
        key: updateRec.key,
        primaryKey: updateRec.primaryKey,
        action: 'update',
        userData: userData,
        recordData: updateRec.record,
        properties: updateRec.properties,
   ...
   onDelete: function(message,userData,callback){
      ...
      if(bucket && key && clientId && record){ 
         var storeRequest = { 
            ...
            userData: userData,
            recordData: record,
            properties: deleteRec.properties,
