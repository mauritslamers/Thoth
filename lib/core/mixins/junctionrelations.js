...
exports.RelationsByJunctionTable = {
  ...
  _retrieveDirectRelation: function(sR,recs,relation,callback){
    ...
    //var isPlural = (relation.isDirectRelation && relation.isMaster && relation.get('isToMany')); 
		var junctionProps = this.getJunctionProperties(sR,relation);
    ...
    var oppositeFieldName = junctionProps.relationRelationKey;
		var relationFieldName = relation.relationKey || junctionProps.modelRelationKey ;
		sys.log('bucket: ' + bucket);
		sys.log('oppositeFieldName: ' + oppositeFieldName);
		sys.log('relationFieldName: ' + relationFieldName);
  ...
  _retrieveJunctionRelation: function(sR,recs,relation,callback){
    ...
    var mainSideFieldName = junctionProps.modelRelationKey;
		var relationSideFieldName = junctionProps.relationRelationKey;
    ...
    var req = API.StoreRequest.create({
      ...
      //bucket: this.junctionTableName(sR.bucket, relation.bucket), 
			bucket: junctionProps.junctionBucket,
  ...
  fetchRelation: function(sR,records,relation,callback){
    ...
    //var relationSideFieldName = this.junctionKeyName(relation.bucket, relation.primaryKey, isPlural);
		var junctionProps = this.getJunctionProperties(sR,relation);
		var mainSideFieldName = relation.relationKey || junctionProps.modelRelationKey;
		var relationSideFieldName = junctionProps.relationRelationKey;
	...
	_createJunctionRecords: function(mainPk,relationPks,junctionProps,callback){
		//if relationPk is not an array, create one record
		//otherwise create multiple records
		var keys, numkeys, count=0, returnKeys = [], me = this;

		var report = function(err,rec){
			count += 1;
			sys.log('report in _createJunctionRecrds called...');
			returnKeys.push(rec[junctionProps.relationRelationKey]);
			if(count === numkeys){
				if(callback){
					sys.log('about to call the callback with ' + sys.inspect(returnKeys));
					callback(err,returnKeys);
				}
			}
		};
		var keySaver = function(key){
			var rec = {};
			rec[junctionProps.relationRelationKey] = key;
			rec[junctionProps.modelRelationKey] = mainPk;
			var req = API.StoreRequest.create({
				bucket: junctionProps.junctionBucket,
				record: rec,
				requestType: C.ACTION_CREATE
			});
			me.createDBRecord(req,{},report);				
		};
	
		keys = (SC.typeOf(relationPks) === 'array')? relationPks: [relationPks];
		numkeys = keys.length;
		keys.forEach(keySaver);	
	},
	
	_createChildRecords: function(childrecs,junctionProps,callback){
		// needed: 
		var me = this;
		var recs, numrecs, keys = [], savedChildRecs = [], count = 0;
		var report = function(err,rec){
			count += 1;
			if(!err && rec){
				keys.push(rec[junctionProps.relationPrimaryKey] || rec.key);
				savedChildRecs.push(rec);
			}
			if(count === numrecs){
				if(callback) callback(null,savedChildRecs);
			}
		};
		var recCreator = function(childrec){
			sys.log('recCreator called');
			var req = API.StoreRequest.create({
				bucket: junctionProps.relationBucket,
				record: childrec,
				requestType: C.ACTION_CREATE
			});
			me.createDBRecord(req,{},report);
		...
		recs = (SC.typeOf(childrecs) === 'array')? childrecs: [childrecs];
		numrecs = recs.length;
		recs.forEach(recCreator);
	},

	// function to fetch junction records
	// mainPks is either one key or multiple keys
	// relationPks is either:
	// - no keys: return all junction records for mainPks
	// - one key: return all junction records for mainPks + relationPks (refresh)
	// - multiple keys: return all junction records for mainPks + relationPks
	// _retrieveJunctionRelation: function(sR,recs,relation,callback){
//     var me = this;
//     var isPlural = (relation.isDirectRelation && relation.isMaster && relation.get('isToMany'));
//     // in a relation from student to exam, relationFieldName is student_id, oppositeFieldName is exam_id
...
// 
//
// if called without relationPks, junction records are requested for all mainPks
// if called without mainPks, relationPks will be mainPks, and all junction records are requested for those
// if called without both mainPks and relationPks all junction records will be returned
	_fetchJunctionRecords: function(mainPks,relationPks,junctionProps,callback){
		var me = this, mainKeys, relationKeys,conds, params;
	  if(!junctionProps) return;	
		mainKeys = (SC.typeOf(mainPks) === 'array')? mainPks: [mainPks];
		relationKeys = (SC.typeOf(relationPks) === 'array')? relationPks: [relationPks];
		if(mainPks && relationPks){
		  conds = junctionProps.modelRelationKey + " ANY {keys} AND " + junctionProps.relationRelationKey + " ANY {relkeys}";
			params = { keys: mainKeys, relkeys: relationKeys };
		}  
		if(!mainPks && relationPks){
			conds = junctionProps.relationRelationKey + " ANY {keys}";
			params = { keys: relationKeys };
		}
		if(mainPks && !relationPks){
		  conds = junctionProps.modelRelationKey + " ANY {keys}";
			params = { keys: mainKeys };
		}
		// if both mainPks and relationPks are undefined, conditions and parameters are also undefined
		var req = API.StoreRequest.create({
			requestType: C.ACTION_FETCH,
			bucket: junctionProps.junctionBucket,
			conditions: conds,
		...
		//sys.log('fetching with conditions: ' + req.conditions);
		this.fetchDBRecords(req,C.SOURCE_THOTH, function(err,recs){
			if(err) callback(err,recs);
			else {
				var filteredRecs = me._filterRecordsByQuery(recs,req.conditions,req.parameters); // make sure records are filtered
				callback(null,filteredRecs);
			}
		});
	},
  ...
  createRelation: function(storeRequest,record,relation,clientId,callback){
		var me = this, mainPk, junctionProps;

    if(!relation.isMaster || relation.isDirectRelation) return false;
    junctionProps = this.getJunctionProperties(storeRequest.bucket,relation.bucket);
    
		mainPk = storeRequest.key || record[storeRequest.primaryKey];
    if(relation.isChildRecord){ 
			// isChildRecord on a indirect relation, means using a junction table and an opposite data table
			// 
			var childprop = relation.propertyName || relation.propertyKey;
			this._createChildRecords(record[childprop],junctionProps,function(err,childrecs){
				sys.log('return function from _createChildRecords: ' + sys.inspect(arguments));
				//now child records are created, create the junction records
			  if(childrecs){
					var keys = childrecs.getEach(relation.primaryKey);
					sys.log('create junctions function is: ' + me._createJunctionRecords.toString());
					sys.log('keys are: ' + sys.inspect(keys));
					me._createJunctionRecords(mainPk,keys,junctionProps,function(err,newjrecs){
						sys.log('return function from _createJunctionRecords: ' + sys.inspect(arguments));
						if(err) callback(err,childrecs);
						else {
							if(relation.type === 'toOne') callback(null,childrecs[0]);
							else callback(null,childrecs);
						}
					});
				}	
			});
				
      //if toOne, create single record and get key, then create relation in the junction table
      //if toMany, create all records and get keys, then create relations in the junction table
    }
    else {
			this._createJunctionRecords(mainPk,relation.keys,junctionProps,function(err,newkeys){
				if(err) callback(err,newkeys);
				else {
					if(relation.type === 'toOne') callback(null,newkeys[0]);
					else callback(null,newkeys);
				}
			});

      //if toOne, create relation using the pk value of the relation
      //if toMany, create relations using the pk values of the relation
    }

    // // original
    // var junctionProps = this.getJunctionProperties(storeRequest.bucket,relation.bucket);
    // 
    ...
    //   newRelRec = {};
    //   newRelRec[junctionProps.modelRelationKey] = masterKey;
    //   newRelRec[junctionProps.relationRelationKey] = relationKeys[i];
    //   // now save by making up a storeRequest
    //   this.createDBRecord({bucket:junctionProps.junctionBucket,key:noKey,recordData:newRelRec},clientId); // don't do callbacks on relations for the moment
    // }
  ...
  destroyRelation: function(storeRequest,relation,clientId,callback){
    if(!relation.isMaster) return false;
    var recKey = storeRequest.key;
    var junctionProps = this.getJunctionProperties(storeRequest.bucket,relation.bucket);
    var me = this;
    var primKey = this.primaryKey || 'id';
    this.fetchDBRecords(junctionProps.junctionBucket,function(err,junctionData){
      // get all junctioninfo for the current record
      var junctionRecs = me._junctionDataFor(storeRequest,junctionProps,junctionData,true); // have it return the entire junction record
      var curJuncKey;
      for(var i=0,len=junctionRecs.length;i<len;i++){
        curJuncKey=junctionRecs[i][primKey];
        me.deleteDBRecord({bucket:junctionProps.junctionBucket,key:curJuncKey},clientId); // fake a storeRequest
      }
	...
	// assemble information about the junction
	getJunctionProperties: function(storeRequest,relation){
		var isPlural = (relation.isDirectRelation && relation.isMaster && relation.get('isToMany')); 
		var ret = {
			modelBucket: storeRequest.bucket,
			relationBucket: relation.bucket,
			modelPrimaryKey: storeRequest.primaryKey,
			relationPrimaryKey: relation.primaryKey,
			junctionBucket: this.junctionTableName(storeRequest.bucket,relation.bucket),
			modelRelationKey: this.junctionKeyName(storeRequest.bucket,storeRequest.primaryKey,isPlural),
			relationRelationKey: this.junctionKeyName(relation.bucket, relation.primaryKey, isPlural)
		};
		return ret;
	},
   
   // function to generate all junction information in one go
//   getJunctionProperties: function(model,relation,modelPrimaryKey,relationPrimaryKey){
// //    in a relation from student to exam, relationFieldName is student_id, oppositeFieldName is exam_id
// //     var mainSideFieldName = this.junctionKeyName(sR.bucket, sR.primaryKey, isPlural);
// //     var relationSideFieldName = this.junctionKeyName(relation.bucket, relation.primaryKey, isPlural);
// //
...
//       junctionBucket: this.junctionTableName(model.bucket,relation.bucket),
//       modelRelationKey: this.junctionKeyName(model.bucket,modelPrimaryKey),
//       relationRelationKey: this.junctionKeyName(relation.bucket,relationPrimaryKey)
//     };
//   },
   ...
   _junctionDataFor: function(record,junctionProps,junctionData,allInfo){
      var i, juncLen=junctionData.length;
      var modelKeyName = junctionProps.modelRelationKey;
      var relationKeyName = junctionProps.relationRelationKey;
   ...
   getRelationKeys: function(relation,record,junctionProps,callback){
      ...
      this.getRelationSet(relation,record,junctionProps,function(relationSet){
   ...
   updateRelation: function(storeRequest,record,relation,clientId,callback){
      ...
      var junctionProps = this.getJunctionProperties(storeRequest,relation);
      //console.log("Junction info: " + JSON.stringify(junctionProps));
      var me = this;
      this.fetchDBRecords({bucket:junctionProps.junctionBucket},{},function(err,junctionData){ 
        // use fetchDBRecords to retrieve relational records
         var relationKeys = relation.keys? Tools.copy(relation.keys): [];
         var junctionRecs = me._junctionDataFor(storeRequest,junctionProps,junctionData,true); // get all info on the records
         var relationsIndex,curRelKey;
         for(var i=0,l=junctionRecs.length;i<l;i++){
            curRelKey = junctionRecs[i][junctionProps.relationRelationKey];
            relationsIndex = relationKeys.indexOf(curRelKey);
            if(relationsIndex == -1){ // not found, so delete the record
               me.deleteDBRecord({bucket: junctionProps.junctionBucket, key:junctionRecs[i][me.primaryKey]}, clientId);
            }
            else {
         ...
         for(var j=0;j<numrelations;j++){
            newRelRec = {};
            newRelRec[junctionProps.modelRelationKey] = masterKey;
            newRelRec[junctionProps.relationRelationKey] = relationKeys[j];
            me.createDBRecord({bucket:junctionProps.junctionBucket,key:noKey,record:newRelRec},clientId); // don't do callbacks on relations for the moment
         }
