  ...
  // relation resolving functions
   ...
   getRelationSet: function(storeRequest,relation,records,junctionInfo,callback){
      ...
      var fetchDBCbDirect = function(err,recordData){
        ...
        var filterProp = function(records,prop,val){
          var ret = [],i,len = records.length;
          for(i=0;i<len;i+=1){
            if(records[i][prop] === val) ret.push(records[i]);
          }
          return ret;
        };
        ...
        var parseRec = function(rec){
          ...
          //tmp = filterProp(recordData,junctionInfo.modelRelationKey,key); // this is MUCH lighter and faster... :(
