...
exports.Store = SC.Object.extend(computedPropertyCalculator,{
   ...
   */
   ...
   uploadDBFile: function(storeRequest,clientId,callback){
      // the callback expects the upload data
      // syntax of callback: callback(err,updatedRecordData);
      sys.log("uploadDBFile: This function needs an implementation. If you are seeing this message, you are probably using the base ThothStore instead of a store having a DB-implementation.");
   },
  ...
  fetch: function(storeRequest,clientId,callback){
    ...
    if(this.isProfiling){
       if(!this._profiler) this._profiler = require('v8-profiler');
       //this._profiler.startProfiling("ThothSC_Store_fetch");
    } 
    ...
    var merge = function(records,relSets){
      ...
      if(me.isProfiling) me._profiler.startProfiling('ThothSC_Store_fetch_merge');
      var relSetMap = function(relSet){
        ...
        if(relSet.keys instanceof Array) relSet.keys.map(relSetRec);
        else relSetRec(relSet.keys);
      };
      relSets.map(relSetMap);
      relationsDidMerge = true;
      if(me.isProfiling) me._profiler.stopProfiling('ThothSC_Store_fetch_merge');
      return records;
    };
    ...
    var calculateCPS = function(){
      if(me.isProfiling) me._profiler.startProfiling('ThothSC_Store_fetch_calculateCPS');
      ...
      if(me.isProfiling) me._profiler.stopProfiling('ThothSC_Store_fetch_calculateCPS');
      finishFetch();
    };
    
    var relationCb = function(relIndex){
      sys.log('relationCb... for ' + sR.bucket);
      return function(data){
        if(me.isProfiling) me._profiler.startProfiling('ThothSC_Store_fetch_relationCB');
        ...
        if(me.isProfiling) me._profiler.stopProfiling('ThothSC_Store_fetch_relationCB');
    ...
    var fetchDBCallback = function(err,recArray){
      ...
      if(!recArray) callback(null);         
        ...
        if(shouldCalculateProperties || sR.combineReturnCalls) recordData = recArray; // save data for cps or in case of combineReturnCalls
        if(!sR.combineReturnCalls) callback({ recordResult: recArray });
        ...
        // if combineReturnCalls is true, but no relations found, make sure the data is sent
...
/*
   ...
   // This could be used if there is a backend to thoth, where uploading 
   // is handled, and the request would be passed through here.
   //
   // (This is not used when uploading is done by Thoth with node-formidable, 
   //  to upload files as a simple service)
   //
   upload: function(storeRequest,clientId,callback){
      if(this.debug) sys.log('ThothStore: upload called');
      var bucket = storeRequest.bucket, key = storeRequest.key, record = storeRequest.recordData;
      var relations = storeRequest.relations, me = this;
      // first update relations, because it allows to be sending the new relation data in one go with the record,
      // which makes distributing the changes much easier. This is because it seems rather tricky to distribute the relation
      ...
      if(this.debug) sys.log("ThothStore: upload: relations handled");
      this.uploadDBFile(storeRequest,clientId,function(err,record){
         // assume data is the updated record
         // merge the relation data with the record
         if(me.automaticRelations && record && relations){
           var currel;
