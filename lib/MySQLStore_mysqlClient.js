...
var mysql = require('./libmysqlclient/mysql-libmysqlclient');
...
exports.MySQLStoreMySQLClient = Store.extend(JunctionMixin, {
      ...
      if(query){
          ...
          if(connection){
             ...
             switch(type){
                ...
                case 'refresh': ret = res.fetchAllSync? res.fetchAllSync(): []; break;
                case 'fetch':  ret = res.fetchAllSync? res.fetchAllSync(): []; break;
                default : ret = res;
             } 
   ...
   recordFieldsInRequest: function(storeRequest){
      ...
      var record = storeRequest.record;
   ...
   createInsertQuery: function(storeRequest){
      ...
      var bucket = storeRequest.bucket, record = storeRequest.record;
   ...
   createDBRecord: function(storeRequest,clientId,callback){
      ...
      if(query){
         ...
         callback(new Error("ThothMySQLStoreMySQLClient: there was an error while trying to create a new record"));
      }
   },
   ...
   updateDBRecord: function(storeRequest,clientId,callback){
      ...
      if(query){
         ...
         callback(new Error("ThothMySQLStoreMySQLClient: Something went wrong while trying to create the query"));
      }
   },
