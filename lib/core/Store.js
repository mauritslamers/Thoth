   ...
   */
   ...
   updateRecord: function(storeRequest,clientId,callback){
      ...
      this.updateDBRecord(storeRequest,clientId,function(record){
         ...
         if(me.automaticRelations && record){
           var currel;
         ...
         callback(record); // now send the record with merged relations, and null if error
      });
   },
   ...
   _filterRecordsByQuery: function(records,conditions,parameters){
      ...
      sys.log('conditions: ' + conditions);
      sys.log('parameters: ' + parameters);
      if(records){
