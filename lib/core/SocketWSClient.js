var	url = require('url'),
		...
		SocketClient = require('./SocketClient').SocketClient;

exports.SocketWSClient = SocketClient.extend({
   ...
   connectionHeartbeat: false,
	...
	_onConnect: function(req, socket){
		...
		sys.puts('ThothSocketWSClient request: ' + sys.inspect(req));
		this.request = req;
		this.connection = socket;
		this.data = '';

        // The websockets protocol updates changed the headers.upgrade string, so there is a problem
        // with handling websockets vs. WebSockets here.
        //
		sys.puts('ThothSocketWSClient this.request.headers.upgrade ' + this.request.headers.upgrade);
		sys.puts('ThothSocketWSClient origin ' + this.request.headers.origin);
		//sys.puts('ThothSocketWSClient verifyOrigin ' + this._verifyOrigin(this.request.headers.origin));
		...
		this.connection.addListener('close', function(){ self._onClose();});
	...
	_write: function(message){
		try {
		  ...
		  sys.log('SocketWSClient: closing connection because unable to write due to ' + sys.inspect(e));
			this._onClose();
		}
	}
