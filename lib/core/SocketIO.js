    ...
    var handlerCaller = function(event,handler){ //if(message.fetch) me.onFetch.call(me,message,userData,returnFunction);
      ...
      return function(data){
        ...
        if(forceAuthentication){
          ...
          // Whoops... trying to give client.userData only works when there is an authentication process
          // which in this case is just not the case. Still, there should be some identifying info 
          // perhaps the sessionKey used by socket.io?
        }
        SC.RunLoop.end();
      };
    };
