...
require('./SocketWSClient');
require('./SocketXHRPollingClient');
var transports = {};
...
global.ThothSocketListener = SC.Object.extend(process.EventEmitter.prototype, {
   ...
   setOption: ThothSocketClient.prototype.setOption, // copy these functions off the Thothsocketclient prototype
   
   setOptions: ThothSocketClient.prototype.setOptions, 
  ...
  start: function(server, options){
		...
		transports['websocket'] = ThothSocketWSClient; // override the websocket 
		transports['xhr-polling'] = ThothSocketXHRPollingClient; // provide the xhr polling client
	...
	*/
	...
	_onClientConnect: function(client){
		if (!(client instanceof ThothSocketClient) || !client.handshaked){
		...
		if(client.isAuthenticated){
         ...
         //this._sendRequestQueue(client);
		   ...
		   sys.log("ThothSocketListener: new unauthenticated client connected");
		}
	         ...
	         if(user && passwd){
	            ...
	            var authCallback = function(authresult){
   	            if(authresult){
   	               ...
   	               sys.log("ThothSocketListener: Client " + client.user + " authenticated, sessionKey: " + client.sessionKey);
   	               return YES; 
   	            }
   	            else {
