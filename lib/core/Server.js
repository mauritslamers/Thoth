...
exports.Server = SC.Object.extend({
  ...
  createHTTPHandler: function(serverObj){
    ...
    var handlers = {
      handleRoot: function(path,method,request,response){
        if(path === '/'){
          if(serverObj.debug){
      ...
      handleAUTHRequest: function(path,method,request,response){
      ...
      handleSession: function(path,method,request,response){
        if(serverObj.forceAuth){
          ...
          if(receivedCookieHeader && receivedUserName){
             ...
             return NO; // proper session info, so don't stop 
          }
          else {
             response.writeHead(403, {'Content-Type':'text/html'});
      ...
      handleRPCGET: function(path,method,request,response){
        var resource;
        if(method === 'GET'){
    ...
    var methodDispatch = function(request,response){
      ...
      if(method !== 'GET'){
        ...
        request.addListener("end", function(){
           serverObj[method](request,reqdata,response); // finish gathering, call delete
        });
      }
      else {
...
// ===========================   
  ...
  _sendHTTPJSONResponse: function(req,res,body){
  ...
  GET: function(request,response){
    ...
    var userData = this.sessionModule.getUserData(receivedUserName);
    ...
    cb = function(records){
      ...
      if(records){
        if(records.recordResult){
          me._sendHTTPJSONResponse(request,response, { fetchResult: { 
             bucket: resource[0], 
             records: records.recordResult }
           });
        }
        else {
          if(records.refreshResult){
            me._sendHTTPJSONResponse(request,response, { 
              refreshRecordResult: { bucket: resource[0], key: resource[1], record: records.refreshResult } 
            });            
          }
          else {
            me._sendHTTPJSONResponse(request,response, API.createErrorReply(Constants.ACTION_REFRESH,Constants.ERROR_DATAINCONSISTENCY,{}));
          }
        }        
      }
      else {
        res = API.createErrorReply(Constants.ACTION_FETCH,Constants.ERROR_DATAINCONSISTENCY,{});
        me._sendHTTPJSONResponse(request,response, res);
      }
    };
    ...
    if(resource.length === 1){ // fetchall
      ...
      me.store.refreshRecord(req,'HTTPREQ',cb);
    }    
  },
  ...
  POST: function(request,data,response){
    var me = this, urlInfo, path, resourcePath, resource, req, cb, storeRequest;
    var receivedUserName = request.headers['username'];
    var urlPrefix = this.getURLPrefix();
    var userData = this.sessionModule.getUserData(receivedUserName);
    ...
    if(data.fetch){
      storeRequest = API.createStoreRequest(fetchinfo,userData,Constants.ACTION_FETCH);
      this.store.fetch(storeRequest,userData,fetchCb);
    }
    else {
      if(data.record){
...
// make sure you use these functions when you are writing a REST interface that should interact with the socket clients!  
 ...
 /*  
   ...
   onFetch: function(message,userData,callback){
      ...
      var fetchRequest = function(policyResponse){
         if(policyResponse){ // if any of YES or "retry"
            ...
            // not allowed
            callback(API.createErrorReply(Constants.ACTION_FETCH, Constants.ERROR_DENIEDONPOLICY,fetchinfo.returnData));
         }
      };
      ...
      if(refreshRec.bucket && refreshRec.key){
         ...
         var refreshAction = function(policyResponse){
            if(policyResponse){ // either 'retry' or YES on first attempt
               ...
               callback(API.createErrorReply(Constants.ACTION_REFRESH, Constants.ERROR_DENIEDONPOLICY,refreshRec.returnData));
            }
         }; // end refreshAction
   ...
   distributeChanges: function(storeRequest, originalUserData){
     ...
     matchingUserSessions = this.sessionModule.getMatchingUserSessionsForRecord(storeRequest);     
   ...
   onCreate: function(message,userData,callback){
      ...
      var storeRequest = API.createStoreRequest(createRec,userData,Constants.ACTION_CREATE);
      if(API.hasInconsistency(storeRequest)){
      ...
      if(storeRequest.bucket && clientId){
         ...
         var createAction = function(policyResponse){
            if(policyResponse){ // either YES or adjusted record
               ...
               me.store.createRecord(storeRequest,clientId,
                  function(rec){
                    if(rec){
                      ...
                      callback(API.createErrorReply(Constants.ACTION_UPDATE, Constants.ERROR_DATAINCONSISTENCY,createRec.returnData));
                    }
                  }
               );
            }
               ...
               callback(API.createErrorReply(Constants.ACTION_CREATE, Constants.ERROR_DENIEDONPOLICY,createRec.returnData));           
            }     
         };
     ...
     if(storeRequest.bucket && storeRequest.key && clientId){
        var updateAction = function(policyResponse){
           if(policyResponse){
              ...
              callback(API.createErrorReply(Constants.ACTION_UPDATE, Constants.ERROR_DENIEDONPOLICY,updateRec.returnData));
           }
        };
      ...
      if(bucket && key && clientId && record){ 
         ...
         var destroyAction = function(policyResponse){
            if(policyResponse){
              ...
              callback(API.createErrorReply(Constants.ACTION_DELETE, Constants.ERROR_DENIEDONPOLICY,deleteRec.returnData));
            }
         };
         if(this.policyModule){
