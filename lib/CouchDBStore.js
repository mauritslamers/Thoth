...
exports.CouchDBStore = Store.extend({
      ...
      return function(){
        db.saveDoc(storeRequest.recordData, function(err,result){
          if(err) sys.log('Error while creating in CouchDB: ' + er);
            ...
            ret.key = result._id; // force a key to be available on the record
            ret._rev = result._rev; // force a revision to be available on the record
            callback(ret); // does couchdb give my record back? no, so use the original data with id and rev data
          }
        });
      };
    };
  ...
  updateDBRecord: function(storeRequest,clientId,callback){
    ...
    db.saveDoc(storeRequest.key, storeRequest.recordData, function(err,result){
      if(err) sys.log('Error while creating in CouchDB: ' + er);
        ...
        var ret = storeRequest.recordData;
        ret._rev = result._rev;
        callback(ret); // does couchdb give my record back? don't rely on it, just get the _rev
      }
    });
  },
  ...
  */
  ...
  updateRelation: function(storeRequest,relation,clientId,callback){
    var recKey = storeRequest.key,
        relKeys = relation.keys.copy(),
        me = this,
        toRemove = [],
        newDoc = {},
        primKey = this.primaryKey,
    ...
    var updateRel = function(db){
      return function(){
        ...
        db.view(me.relationDesignName,modelBucket,null,function(er,result){
          if(er) sys.log('CouchDB updateRelation error with ' + junctionBucket + ' :' + sys.inspect(er));
          else {
