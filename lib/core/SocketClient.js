...
exports.SocketClient = SC.Object.extend({
	...
	_onMessage: function(data){
		...
		for (var i=0,l=messages.length;i<l;i++){
            sys.log('this.listener._onClientMessage() called for ' + sys.inspect(messages[i]));
	...
	_onClose: function(){
		...
		//this._disconnectTimeout = setTimeout(timeOutFunc, this.options.closeTimeout || this.closeTimeout || 500);
		this._onDisconnect();
	},
	...
	_onDisconnect: function(){	
		...
		if (!this.finalized){
			...
			sys.log('Thoth SocketClient: Oops, client disconnect while already finalized?');
			//if(this.handshaked){
				this.listener._onClientDisconnect(this);
			//}
		}
