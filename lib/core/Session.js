...
exports.Session = SC.Object.extend({
  ...
  checkSession: function(userData, callback){ // checking session 
    ...
    var req = this._createStoreRequest(user,sesKey,Constants.ACTION_REFRESH);
    this.store.refreshRecord(req,{},function(err,ret){
      var rec = ret.refreshResult;
      ...
      if(!rec){
        ...
        if((now - rec.lastSeen) > timeout){ // timeout, so destroy
          ...
          rec.lastSeen = now;
          request = me._createStoreRequest(user,sesKey,Constants.ACTION_UPDATE,rec);
          me.store.updateRecord(request,{},function(err,rec){ // other callback signature?
            sys.log('arguments on updateRecord: ' + sys.inspect(arguments));
  ...
  destroySession: function(user,sessionKey,callback){ // destroy session, when logging out for example
    var me = this;
    var req = this._createStoreRequest(user,sessionKey,Constants.ACTION_REFRESH);
    this.store.refreshRecord(req,{},function(err,ret){
      var rec = ret.refreshResult;
      var delreq;
      if(!rec){
  ...
  storeBucketKey: function(user,sessionKey,bucket,key){
    
  },
  
  storeRecords: function(user,sessionKey,record){
  ...
  _storeQueryInCache: function(sessionKey,bucket,conditions,parameters){
    var q, 
        b = bucket,
        c = conditions || "_ALLCONDS_",
        p = parameters? JSON.stringify(parameters): "_ALLPARAMS_";
