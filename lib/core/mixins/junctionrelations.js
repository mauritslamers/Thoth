...
exports.RelationsByJunctionTable = {
  ...
  fetchRelation: function(sR,records,relation,callback){
	...
	_createJunctionRecords: function(mainPk,relationPks,junctionProps,callback){
		...
		var report = function(err,rec){
			...
			if(count === numkeys){
				if(callback) callback(err,returnkeys);
			}
		};
  ...
  createRelation: function(storeRequest,record,relation,clientId,callback){
    ...
    var createChildRecs = function(childrecs,cb){
      var recs = (SC.typeOf(childrecs) === 'array')? childrecs: [childrecs];
			sys.log('trying to create child records for relation');
			...
			var report = function(err,rec){
				count += 1;
				sys.log('report for recCreator called');
				if(rec){
				  keys.push(rec[relation.primaryKey] || rec.key); // save for junction record creation
          crs.push(rec);
				} 
				if(count === numrecs){
					createJunctionRecs(keys,function(err,k){
						sys.log('callback of createJunctionRecs in recCreator called');
						if(cb) cb(null,crs); // create 
						else sys.log('no callback detected in createJunctionRecs callback in recCreator');
					});
				}
			};
			var recCreator = function(childrec){
		...
		var mainPk = storeRequest.key || record[storeRequest.primaryKey];
    if(relation.isChildRecord){ 
  ...
  destroyRelation: function(storeRequest,relation,clientId,callback){
   ...
   updateRelation: function(storeRequest,record,relation,clientId,callback){
	...
	_fetchJunctionRecords: function(){
	
	},

	_createJunctionRecord: function(){
		
	},

	_deleteJunctionRecord: function(){
												
	},

	_fetchChildRecords: function(){
	},

	_createChildRecord: function(){
	},

	_updateChildRecord: function(){
	
	},

	_deleteChildRecord: function(){
