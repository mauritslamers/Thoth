...
exports.SocketIO = SC.Object.extend({
  ...
  _reauthWrapper: function(client){
    ...
    return function(data){
      ...
      if(user && sK && sesMod) {
        sys.log("valid data, checking session...");
        sesMod.checkSession(user,sK,skOnly,function(hasSession){
          ...
          if(hasSession){
            client.userData = { username: user, sessionKey: sK };
            client.user = user;
  ...
  _attachHandlers: function(client,context){
    ...
    var handlerCaller = function(handler){ //if(message.fetch) me.onFetch.call(me,message,userData,returnFunction);
      return function(data){
        if(!client.userData){ 
          sys.log('trying to call a handler without user data... this is weird...');
          sys.log('data: ' + sys.inspect(data,false,5));
          //sys.log('client: ' + sys.inspect(client,false,5));
          //sys.log('data: ' + sys.inspect(data,false,5));
          return; // don't handle
        }
