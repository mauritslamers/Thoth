...
APIRequest.from = function(obj,source,requestType){  
  var json, schema,res,ret,      
      API = require("../API"),
      jsvEnv = API.JSV.createEnvironment();
  
  if(obj){
    json = (obj.instanceOf && obj.instanceOf(StoreRequest))? obj.get('json'): obj; 
    schema = API.APISCHEMAS[C.ACTION_FETCH];
    res = jsvEnv.validate(json,schema);
    if(res.errors.length === 0){
      ret = APIRequest.create(obj, { source: source, requestType: requestType });
    }
    else {
