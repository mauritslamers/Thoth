...
exports.SocketListener = SC.Object.extend(process.EventEmitter.prototype, {
   ...
   authenticatedClients: null, // array of authenticated clients
   
   unAuthenticatedClients: null, // array of clients not yet authenticated
  ...
  start: function(server, options){
	...
	*/
	...
	getClientBySessionKey: function(sessionKey){
		...
		// disconnected client
		var numclients = this.authenticatedClients.length;
		var curClient;
		//console.log('num of clients: ' + numclients);
		for(var i=0;i<numclients;i+=1){
			curClient = (this.authenticatedClients[i].sessionKey == sessionKey)? this.authenticatedClients[i]: curClient;
