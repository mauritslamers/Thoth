...
var querystring = require('querystring'), // for session key parsing
    ...
    UploadHooks = require('./UploadHooks'),
    im = require('../node-imagemagick/imagemagick'),
...
exports.Server = SC.Object.extend(httpResponses,StoreActionCreators,{
	...
	init: function(){
		...
		if(!this.uploadURL) this.uploadURL= '/upload'; // setting a default value when an instance is made
	},
	...
	start: function(){
		...
		if(!this.tempStore) this.tempStore = DiskStore.create();
  ...
  calculateThumbnailData: function(imageWidth, imageHeight, canvasWidth, canvasHeight) {
	...
	_attachSocketListener: function(){
		...
		this.socketIO.start(this.server,{
			...
			onClientMessage: function(message, client){
				...
				if(me.isProfiling) me._profiler.startProfiling("onClientMessage");
				SC.RunLoop.begin(); 
				if(me.debug){
				...
				if(message.uploadRequest) me.onUploadRequest.call(me,message,userData,returnFunction);
				if(message.uploadPollRequest) me.onUploadPollRequest.call(me,message,userData,returnFunction);
				if(message.logOut) me.onLogout.call(me,message,userData,returnFunction);
				SC.RunLoop.end();
				if(me.isProfiling) me._profiler.stopProfiling("onClientMessage");
			}
    ...
    // the order of the function names is the order of execution
		...
		var handlers = {
            ...
            handleUploadPOST: function(path,method,request,response,callback){
              var resource;
              if(method === 'POST'){
                resource = path.slice(1);
                if(resource && (resource.indexOf('upload') === 0)){
                  serverObj.UPLOAD(request,resource,response);
			      callback('handleUploadPOST', true);
                }
                else callback('handleUploadPOST', false);
              }
			  else callback('handleUploadPOST', false);
            },

			handleUploadProgressGET: function(path,method,request,response,callback){
			  var resource;
			  if(method === 'GET'){
			    resource = path.slice(1);
			    if(resource && (resource.indexOf('progress') === 0)){
			      serverObj.UPLOAD_PROGRESS(request,response);
			      callback('handleUploadProgressGET',true);
			    }
  			  else callback('handleUploadProgressGET',false);
			  }
			  else callback('handleUploadProgressGET',false);
			}
 ...
 */
   ...
   UPLOAD: function(request,resource,response){
      // UPLOAD accepts a multipart request
      // call the UPLOAD
      // split the cacheKey from the resource
      sys.log("ThothServer: UPLOAD called");
      var resourceInfo = resource.split('/');

      if(resourceInfo && (resourceInfo.length===2)){
        var cacheKey = resourceInfo[1];
        if(cacheKey && this.uploadModule){
          var me = this;
          var path = url.parse(request.url).pathname;
          ...
          if(parts.length === 2){ // if not, someone is playing with us... and we should ignore it
            ...
            //sys.log('key |' + key + '|');
            this.tempStore.refreshDBRecord(this._createTempURLStoreRequest(bucket,key),null,function(what, rec){
              if (rec) { // something found, so parse it
                //sys.log('rec ' + rec);
                if (me._checkTempURLValidity.call(me,bucket,key,rec)) { // function auto-updates tempStore
                  // valid url, so perform upload
                  me.uploadModule.performUpload(request, function(data){
                    if (data) {
//
...
//
                        ...
                        img32.onload = function(){
                          var canvas = new Canvas(32, 32), 
                              ...
                              td = me.calculateThumbnailData(data.imageFeatures.width, data.imageFeatures.height, 32, 32);
                          
                          ctx.drawImage(img32, td.x, td.y, td.resizeWidth, td.resizeHeight);
                          canvas.toBuffer(function(err, buf){
                            var outfile32 =  'tn' +  32 + 'x' +  32 + '_' + data.filename;
                            fs.writeFile('./upload/' + outfile32, buf, function(){
                        ...
                        img64.onload = function(){
                          var canvas = new Canvas(64, 64), 
                              ctx = canvas.getContext('2d'),
                              td = me.calculateThumbnailData(data.imageFeatures.width, data.imageFeatures.height, 64, 64);
                          
                          ctx.drawImage(img64, td.x, td.y, td.resizeWidth, td.resizeHeight);
                          canvas.toBuffer(function(err, buf){
                            var outfile64 =  'tn' +  64 + 'x' +  64 + '_' + data.filename;
                            fs.writeFile('./upload/' + outfile64, buf, function(){
                        ...
                        img128.onload = function(){
                          var canvas = new Canvas(128, 128), 
                              ctx = canvas.getContext('2d'),
                              td = me.calculateThumbnailData(data.imageFeatures.width, data.imageFeatures.height, 128, 128);
                          
                          ctx.drawImage(img128, td.x, td.y, td.resizeWidth, td.resizeHeight);
                          canvas.toBuffer(function(err, buf){
                            var outfile128 =  'tn' +  128 + 'x' +  128 + '_' + data.filename;
                            fs.writeFile('./upload/' + outfile128, buf, function(){
                        ...
                        img256.onload = function(){
                          var canvas = new Canvas(256, 256), 
                              ctx = canvas.getContext('2d'),
                              td = me.calculateThumbnailData(data.imageFeatures.width, data.imageFeatures.height, 256, 256);
                          
                          ctx.drawImage(img256, td.x, td.y, td.resizeWidth, td.resizeHeight);
                          canvas.toBuffer(function(err, buf){
                            var outfile256 =  'tn' +  256 + 'x' +  256 + '_' + data.filename;
                            fs.writeFile('./upload/' + outfile256, buf, function(){
                        ...
                        img512.onload = function(){
                          var canvas = new Canvas(512, 512), 
                              ctx = canvas.getContext('2d'),
                              td = me.calculateThumbnailData(data.imageFeatures.width, data.imageFeatures.height, 512, 512);
                          
                          ctx.drawImage(img512, td.x, td.y, td.resizeWidth, td.resizeHeight);
                          canvas.toBuffer(function(err, buf){
                            var outfile512 =  'tn' +  512 + 'x' +  512 + '_' + data.filename;
                            fs.writeFile('./upload/' + outfile512, buf, function(){
                              ...
                              response.write(JSON.stringify( { mimeType: 'text/url', imageURL: '/upload/' + data.filename } ));
                              response.end();
                            });
                          });
                        };
                      ...
                      me.send404(response); // force the end of the response if nothing can be found?
                    }
                  });
                  return;
                }
      ...
      */
      ...
      var callback = function(authResult, message){
        if(authResult){
          ...
          sys.log("about to reply with: " + JSON.stringify(ret));
  ...
  UPLOAD_PROGRESS: function(request,response){
    var uploadModule = this.uploadModule,
        value = 0.0;

    if (uploadModule) value = uploadModule.uploadProgress;

    sys.log('upload progress value is ' + value);

    //this.send200(response,this.uploadModule.uploadProgress,"text/html");
    response.writeHead(200, {'Content-Type': 'application/json' });
    response.write(JSON.stringify( { mimeType: 'text/html', uploadProgress: value } ));
    response.end();
  },
  ...
  registerTemporaryURL: function(bucket,key,data,mimeType,limit,limitType,shouldAnonymize){
    ...
    sys.log('Thoth Server: registering temporary in temporary store');
    this.tempStore.createDBRecord(this._createTempURLStoreRequest(bucket,key,record));
    return true; // to get rid of strict warning about no return
  },
    ...
    var determineActionType = function(sessionInfo){
      ...
      switch(whatToDo){
        ...
        case (Constants.ACTION_DELETE+Constants.DISTRIBUTE_QUERY): // do nothing, as the record should not exist at the client
          break;
        default: 
          sys.log('Distribute records: default whatToDo should not happen at all. Major error!!');
          break;
      }         
    };
            ...
            switch(action){
               ...
               case 'delete':
                  ...
                  if(curMatchType == 'query'){
                     // do nothing
                     sys.log('curMatchType == query, so doing nothing');
                  }
                  break;
               default: // whoops?? 
   ...
   */
    ...
    onUploadRequest: function(message, userData, returnFunction) {
      var me = this;
      //sys.log("ThothServer: onUploadRequest called");
      if (this.uploadModule) {
        var uploadRequest = message.uploadRequest;
        if (uploadRequest) {
          //sys.log("ThothServer: received Upload request from client: " + userData.username);
          var fn = uploadRequest.functionName;
          var params = uploadRequest.params;
          //sys.log('fn is ' + sys.inspect(fn));
          //sys.log('params are ' + sys.inspect(params));
          
          this.uploadModule.callUploadFunction(
            fn,
            params,
            function(data) {      // this callback will register tmp url, if permission granted, then will call returnFunction
              data.uploadRequestResult.returnData = uploadRequest.returnData;
              var permission = data.uploadRequestResult.record.permission;
              //sys.log('permission ' + permission);
              if (permission === 'granted') {
                //sys.log('permission granted ' + data.uploadRequestResult.cacheKey);
                me.registerTemporaryURL(
                  'upload',
                  data.uploadRequestResult.cacheKey,
                  {},
                  'application/json',
                  0,
                  'num',
              ...
              //sys.log('data is ' + sys.inspect(data));
              returnFunction(data); // the data contains data.uploadRequestResult.uploadURL if permission granted
            });
        }
      }
    },

    onUploadPollRequest: function(message, userData, returnFunction) {
      var me = this;
      //sys.log("ThothServer: onUploadPollRequest called");
      if (this.uploadModule) {
        var uploadPollRequest = message.uploadPollRequest;
        if (uploadPollRequest) {
          //sys.log("ThothServer: received Upload Poll request from client: " + userData.username);
          var fn = uploadPollRequest.functionName;
          var params = uploadPollRequest.params;
          //sys.log('fn is ' + sys.inspect(fn));
          //sys.log('params are ' + sys.inspect(params));
