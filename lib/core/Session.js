...
exports.Session = SC.Object.extend({
  ...
  createSession: function(userData){ // create a new session
    ...
    var createSessionKey = function(){
      ...
      if(!me._sessionsKeys[key]) return key;
    ...
    this.store.createRecord(sr, {}, function(rec){
      if(!rec) sys.log('Session: major error, not able to create records in the session store');
    }); // we should be able to catch errors here, because this would be severe...
    this._sessionsKeys[newSessionKey] = 1; // should evaluate to true
  ...
  destroySession: function(user,sessionKey){ // destroy session, when logging out for example
