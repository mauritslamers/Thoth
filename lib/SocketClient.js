...
global.ThothSocketClient = SC.Object.extend({
   ...
   userData: null, // the user data as known to the authentication system
	...
	send: function(message){
	   //sys.log("Send called on ThothSocketClient with message " + sys.inspect(message));
	   //sys.log('ThothSocketClient: send: check on options: ' + (!this.isConnected || !(this.connection.readyState === 'open' || this.connection.readyState === 'writeOnly')) )
	   //sys.log('ThothSocketClient: send: this.isConnected: ' + this.isConnected);
	   //sys.log('this.connection: ' + sys.inspect(this.connection));
	   //sys.log('ThothSocketClient: this.connection.readyState: ' + this.connection.readyState);
