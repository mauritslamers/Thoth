...
global.SC.Query = SC.Object.extend(SC.Copyable, SC.Freezable, 
  ...
  */
  parse: function() {
    ...
    if (tree && tree.error){
      SC.Logger.log("Query error: conditions: " + this.get('conditions'));
      SC.Logger.log('Query parameters: ' + SC.inspect(this.get('parameters')));
      throw tree.error;
    } 
    ...
    'CONTAINS': {
      ...
      /** @ignore */
        evaluate:       function (r,w) {
                          ...
                          if (all instanceof SC.RecordArray) all = all.toArray();
