...
global.OrionMySQLStoreMySQLClient = OrionStore.extend({
   ...
   _hasConnection: function(){
      // only used as getter
      if(this._connection && this._connection.connectedSync){
         return this._connection.connectedSync();
      }
      else return NO;
   }.property(),
   ...
   escapeField: function(value){
      ...
      if(value instanceof String){
         ...
         try {
            ret = this._connection.escapeSync(value);
   ...
   performQuery: function(query){
      if(!this.get('_hasConnection')) this.createConnection(); // try to create the connection if there isn't one
      if(this.get('_hasConnection') && query){
         //sys.log('OrionMySQLStoreMySQLClient: trying to perform query: ' + query)
         var res;
         try{
            res = this._connection.querySync(query);            
         } catch(e) { 
            ...
            sys.log("OrionMySQLStoreMySQLClient: MySQL error message: " + this._connection.errorSync());
         }
         var ret;
         if(res.fetchAllSync) ret = res.fetchAllSync();
         else ret = res;
         return ret;
      }
      else return NO;
   ...
   setTableCache: function(){
      ...
      if(this.get('_hasConnection')){
         ...
         if(tables){
            ...
            sys.log("OrionMySQLStoreMySQLClient: setTableCache finished");
            return YES;    
         }
      }
      // in case of error;
   ...
   recordFieldsInRequest: function(storeRequest){
      ...
      // else it returns NO
      //sys.log("OrionMySQLStoreMySQLClient: trying to get the table fields in the request");
   ...
   createDBRecord: function(storeRequest,clientId,callback){
      ...
      if(query){
         ...
         var er = this._connection.errorSync? this._connection.errorSync(): NO;
         if(!er){
            if(callback){
               sys.log("About to request last insert id");
               var newId = this._connection.lastInsertIdSync();
               sys.log("last insert id requested: " + newId);
               var ret = this.filterRecord(storeRequest);
               sys.log("Filtered request");
            ...
            sys.log("OrionMySQLStoreMySQLClient: Error while performing createDBRecord " + er);
            sys.log("OrionMySQLStoreMySQLClient: Query: " + query);
   ...
   updateDBRecord: function(storeRequest,clientId,callback){
      ...
      if(query){
         ...
         else sys.log('OrionMySQLStoreMySQLClient: error: ' + er);
      }
      else {
   ...
   deleteDBRecord: function(storeRequest,clientId,callback){
      ...
      try {
         ...
         sys.log("OrionMySQLStoreMySQLClient: Something went wrong while doing a query. " + e); 
         sys.log("OrionMySQLStoreMySQLClient: Query: " + query); 
         sys.log("OrionMySQLStoreMySQLClient: MySQL error message: " + this._connection.errorSync());
      }
