...
exports.Store = SC.Object.extend(computedPropertyCalculator,{
   ...
   // server is Thoth Server instance
   // callback is the option callback when the init of the store has been completed.
  ...
  fetch: function(storeRequest,clientId,callback){
    ...
    var finishFetch = function(){
      ...
      if(sR.combineReturnCalls){ 
        ...
        callback(null,{ recordResult: recordData }); // all relation data and CPS merged onto the data
        ...
        if(shouldCalculateProperties){
          ...
          callback(null,{ computedPropertyResult: { bucket: storeRequest.bucket, keys: cpsResult.keys, data: cpsResult.data }});
        }        
      }
    ...
    var relationCb = function(relIndex){
      ...
      return function(data){
        ...
        if(!sR.combineReturnCalls) callback(null,{ relationSet: data }); // first give back the info
    ...
    var handleRelations = function(records){
      var relations = sR.relations,
          relation, i, len = relations.length;
      ...
      if(!me.fetchRelation) sys.log("Thoth.Store: No relation mixin or fetchRelation implementation found");
        ...
        for(i=0;i<len;i+=1){  // for every relation, get the data 
          relation = relations[i];
          if(relation.get('isRetrievable')){
            me.fetchRelation(storeRequest,records,relation,relationCb(i));
          } 
        }
      }
    };
    ...
    var fetchDBCallback = function(err,recArray){
      ...
      if(!recArray) callback(null);         
        ...
        if(!sR.combineReturnCalls){
          ...
          callback(err,{ recordResult: records });
        } 
    ...
    else sys.log('WARNING: Store fetch function called without bucket or callback...');
  },
   ...
   refreshRecord: function(storeRequest,clientId,callback){
      ...
      this.refreshDBRecord(storeRequest,clientId,function(err,record){
        ...
        if(record){ // it can happen that the db sends a null value, or an undefined if the record doesn't exist
          ...
          callback(null,{ refreshResult: record });  
          // do relations
          relations = storeRequest.relations;
          if(me.automaticRelations && relations && (relations instanceof Array)){
            numRels = relations.length;
            sys.log('trying to retrieve relations for refresh...');
            var cb = function(data){
              callback(null,{relationSet: data });
            };
            
            for(i=0;i<numRels;i++){
              //this.getRelationSet(relations[i],storeRequest,junctionInfo,callback);
              if(relations[i].get('isRetrievable')) me.fetchRelation(storeRequest,[record],relations[i],cb);
              else { // send back empty relSet, better make the API (and ThothSC behave better... :) )
                callback(null,{ 
                  relationSet: { 
                    keys: [], data: {}, propertyName: relations[i].propertyName, propertyKey: relations[i].propertyKey
                  }
                });
              }
            }
   ...
   createRecord: function(storeRequest,clientId,callback){
      ...
      this.createDBRecord(storeRequest,clientId,function(err,newrec){
         ...
         if(callback) callback(err,newrec);
      });
   },
   ...
   updateRecord: function(storeRequest,clientId,callback){
      ...
      if(this.automaticRelations && relations && (relations instanceof Array)){
         for(var i=0,l=relations.length;i<l;i++){
           if(!relations[i].isDirectRelation) this.updateRelation(storeRequest,record,relations[i],clientId); 
      ...
      this.updateDBRecord(storeRequest,clientId,function(err,record){
         ...
         if(callback) callback(err,record); // now send the record with merged relations, and null if error
      });
   },
   
   deleteRecord: function(storeRequest,clientId,callback){
     //sys.log('basic store deleteRecord...');
      var bucket = storeRequest.bucket, key=storeRequest.key, relations = storeRequest.relations;
      // first destroy relations
      if(this.automaticRelations && relations && (relations instanceof Array)){
         for(var i=0,len=relations.length;i<len;i++){
           if(!relations[i].isDirectRelation) this.destroyRelation(storeRequest,relations[i],clientId); // for the moment, don't provide a callback
         }
      }
