...
exports.SocketXHRPollingClient = SocketClient.extend({
  ...
  handlePOST: function(req,res){
    ...
    req.addListener('end', function(){
      sys.log("ThothXHR End of POST Request...");
      try {
        var msg = qs.parse(body);
        sys.log("XHRPolling POST received with msg: " + sys.inspect(msg));
	...
	hasValidSession: function(req,res,user,sessionKey){
	  this.user = user;
    this.sessionKey = sessionKey;
    this.isAuthenticated = YES;
    this.userData = this.ThothServer.sessionModule.getUserData(user,sessionKey);
    //sys.log('inspecting hasValidSession: ' + sys.inspect(this.userData));
    
    if(req.method === 'GET') this.handleGET(req,res);
    if(req.method === 'POST') this.handlePOST(req,res);
	},
	...
	sendRequestQueue: function(){
	   ...
	   var me = this;
	   this.ThothServer.sessionModule.retrieveRequestQueue(user,sessionKey,function(queue){
	     if(queue && (queue instanceof Array) && (queue.length > 0)){
	       me._write(JSON.stringify(queue));
	     }
	   });
	}
