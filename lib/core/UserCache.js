...
UserCache: module for storing session related data
This could just be a simple 
...
exports.UserCache = SC.Object.extend({
   ...
   // in an attempt to get rid of a memory leak, explicitly clean out this class
   destroy: function(){
     var i;
     for(i in this._bucketKeyStore){
       if(this._bucketKeyStore.hasOwnProperty(i)) delete this._bucketKeyStore[i];
     }
     delete this._bucketKeyStore;
     this._fetchQueryStore.forEach(function(v){
       v.destroy();
     });
     delete this._fetchQueryStore;
     for(i in this._requestQueue){
       if(this._requestQueue.hasOwnProperty(i)) delete this._requestQueue[i];
     }
