...
exports.Session = SC.Object.extend({
  ...
  */
  ...
  init: function(){
    ...
    if(!this.store){
      this.store = DiskStore.create({ autoRestore: true, useAutoIncrementIndex: false, filename: 'sessionData.js'});
    }
  ...
  saveSessionObject: function(username,sessionKey,record,callback){
    ...
    var sr = API.createStoreRequest({ bucket: username, key: sessionKey, recordData: record}, this._ud, Constants.ACTION_UPDATE);
  ...
  createSessionObject: function(username,sessionKey,userData,callback){
    ...
    var sr = API.createStoreRequest({ bucket: username, key: sessionKey, recordData: rec}, this._ud, Constants.ACTION_CREATE);
