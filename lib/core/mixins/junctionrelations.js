...
exports.RelationsByJunctionTable = {
  ...
  createRelation: function(storeRequest,record,relation,clientId,callback){
    ...
    var createJunctionRecs = function(relationPk, cb){
      //if relationPk is not an array, create one record
      //otherwise create multiple records
      var keys = (SC.typeOf(relationPk) === 'array')? relationPk: [relationPk];
      var numkeys = keys.length;
      var count = 0;
      var returnKeys = [];
      var report = function(err,rec){
			  sys.log('reporter in createJunctionRecs');
        count += 1;
        returnKeys.push(rec[junctionInfo.relationRelationKey]);
        if(count === numkeys){
          if(cb) cb(err,returnKeys);
        }
      };
      var keySaver = function(key){
        var rec = {};
        rec[junctionInfo.modelRelationKey] = record[storeRequest.primaryKey] || record.key;
        rec[junctionInfo.relationRelationKey] = key;
        var req = API.StoreRequest.create({
          bucket: junctionInfo.junctionBucket,
          record: rec,
    ...
    var createChildRecs = function(childrecs,cb){
      var recs = (SC.typeOf(childrecs) === 'array')? childrecs: [childrecs];
          ...
          crs.push(rec);
			...
			var recCreator = function(childrec){
			  sys.log('recCreator called');
				var req = API.StoreRequest.create({
					bucket: junctionInfo.relationBucket,
					record: childrec,
					requestType: C.ACTION_CREATE
				});
				me.createDBRecord(req,{},report);
		...
		var filterExisting = function(keys,
    ...
    if(relation.isChildRecord){ 
			...
			createChildRecs(record[childprop],function(err,newrecs){
				if(err) callback(err,newrecs);
				else {
					if(relation.type === 'toOne') callback(null,newrecs[0]);
					else callback(null,newrecs);
				}
			});
      ...
      //if toMany, create all records and get keys, then create relations in the junction table
			...
			createJunctionRecs(relation.keys,function(err,newkeys){
